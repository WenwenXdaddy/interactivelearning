...
## Follow-up: inline return links on cited paragraphs (2026-09-17, second push)

User feedback after the first push: after "在全文中查看这一段" the target paragraph had no return link of its own; only the sidebar's "返回上一位置" existed. A small inline script (appended before `</body>`, plus one `<style>` block) now appends a "← 站 NN" button to every cited paragraph at load, built from the existing jump elements (`a.to-source[href^="#src-"]` with `data-origin`, or `button.source-jump[data-source-line]` with `data-return-step`). Clicking it shows the originating station and scrolls to that step. No existing script, ID, storage key or text was changed; `public/_headers` hashes regenerated by the builder.

Browser acceptance (Python Playwright + Chromium, file://, 1280×720): for each of the seven courses the backlink count equals the jump count (62 / 56 / 57 / 63 / 64 / 61 / 78); a jump from station 2 lands on the cited paragraph with the backlink attached; clicking it returns to the originating step with that step in view; zero page errors; no horizontal overflow at 390 / 360 on the source station. `npm test`: 106 checks passed. Screenshots: `evidence/upgrade-seven-toc/screenshots/*-backlink.png`.
