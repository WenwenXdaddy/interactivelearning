...
## 14. 第三轮：居中封面与两侧留白（2026-09-23）

用户反馈封面之间过密。§3.3 的映射改为经典 Cover Flow 几何，参数集中在 `home.js` 的 `SHELF_*` 常量，偏移量按封面宽度比例缩放，桌面与手机自动适配：

- **倾斜提前饱和**：`turned = min(1, |d| / 0.3)`，相邻封面即达到最大角 **55°**（原 42° 在 d=1 才到达），居中封面保持正面。
- **两侧外推**：`--tx = sign(d) · 0.46 · coverW · turned − 0.13 · coverW · d`。前一项把两侧封面推离中心，后一项把远端封面略向内收拢以保持堆叠紧凑。
- **景深**：`--tz = −120px · turned`。
