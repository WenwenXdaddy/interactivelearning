...
## 第三轮修正：吸附点偏移（2026-09-23）

改动：仅 `content/portal/home.css`，变换与压暗层从 `li.shelf-item` 移到 `a.cover`（spec §15）。

- 复现（改前，本地 Chromium 1440）：点击「上一门」后 scrollLeft 保持 1536 不变；`scrollTo(1344, instant)` 读回立即为 1536，证明是吸附重吸而非脚本问题。
